 <?php
/**
 * Home template — used for the designated Posts page (e.g. Resources).
 * WordPress automatically uses home.php when a static front page is set and this is the posts index.
 *
...
$page_title    = $posts_page_id ? get_the_title( $posts_page_id ) : 'Resources';
?>
...
<main class="ff-archive-page">
	...
	<header class="ff-archive-header">
		<div class="ff-archive-header__inner">
			<h1 class="ff-archive-header__title"><?php echo esc_html( $page_title ); ?></h1>
			<?php
			if ( $posts_page_id ) {
				$page = get_post( $posts_page_id );
				if ( $page && ! empty( $page->post_content ) ) :
					?>
					<div class="ff-archive-header__desc"><?php echo wp_kses_post( apply_filters( 'the_content', $page->post_content ) ); ?></div>
				<?php
				endif;
			}
			?>
			<nav class="ff-archive-pills" aria-label="Category filter">
				...
				?>
				<a href="<?php echo esc_url( home_url( '/resources/' ) ); ?>" class="ff-archive-pill ff-archive-pill--active">All</a>
				<a href="<?php echo $parish_cat ? esc_url( get_category_link( $parish_cat ) ) : esc_url( home_url( '/category/parish/' ) ); ?>" class="ff-archive-pill ff-archive-pill--inactive">Parish</a>
				<a href="<?php echo $home_cat ? esc_url( get_category_link( $home_cat ) ) : esc_url( home_url( '/category/home/' ) ); ?>" class="ff-archive-pill ff-archive-pill--inactive">Home</a>
				<a href="<?php echo $heart_cat ? esc_url( get_category_link( $heart_cat ) ) : esc_url( home_url( '/category/heart/' ) ); ?>" class="ff-archive-pill ff-archive-pill--inactive">Heart</a>
			</nav>
		</div>
	</header>
